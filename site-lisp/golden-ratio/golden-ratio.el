...
(defcustom golden-ratio-wide-adjust-factor 0.8
  "Width adjustment factor for widescreens.
Used when toggling between widescreen and regular modes."
  :group 'golden-ratio
  :type 'float)

(defcustom golden-ratio-auto-scale nil
  "Automatic width adjustment factoring.
Scales the width of the screens to be smaller as the frame gets bigger."
  :group 'golden-ratio
  :type 'boolean)
...
(defun golden-ratio-adjust (a)
  "Set the adjustment of window widths as A."
  (interactive
   (list
...
(defun golden-ratio--resize-window (dimensions &optional window)
  "Resize the WINDOW or `selected-window' to `golden-ratio' DIMENSIONS."
  (setq window (or window (selected-window)))
  (with-selected-window window
    (let ((nrow  (floor (- (cl-first  dimensions) (window-total-height window))))
          (ncol  (floor (- (cl-second dimensions) (window-total-width window)))))
      (when (window-resizable-p window nrow)
        (enlarge-window nrow))
      (when (window-resizable-p window ncol t)
        (enlarge-window ncol t)))))

(defun golden-ratio-exclude-major-mode-p ()
  "Return non-nil if `major-mode' should not use `golden-ratio'."
...
Optional argument ARG 1."
  (interactive "p")
  (unless (or (not golden-ratio-mode)
              ;; (null arg) ;; investigate the significance of this
              (window-minibuffer-p)
              (one-window-p)
    ...
    (let ((dims (golden-ratio--dimensions))
      ...
      (let ((window-configuration-change-hook nil)
        ...
        (balance-windows)
        ;; (mapc (lambda (w) (window-resize w 0))
        ;;       (window-list))
        (golden-ratio--resize-window dims)
        ;; TODO: fix fill-column-width type window recenterring
...
;; Should return nil
(defadvice other-window (after golden-ratio-resize-window)
  (golden-ratio)
  nil)

;; Should return the buffer
(defadvice pop-to-buffer (around golden-ratio-resize-window)
...
(defun golden-ratio--post-command-hook ()
  (when (or (memq this-command golden-ratio-extra-commands)
    ...
    (run-with-idle-timer 0.01 nil 'golden-ratio)))
