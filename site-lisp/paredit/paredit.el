...
\\<paredit-mode-map>"
  ...
  (when (and paredit-mode
    ...
    (condition-case condition
        (check-parens)
      (error
       (when (not (funcall paredit-override-check-parens-function condition))
         (setq paredit-mode nil)
         (signal (car condition) (cdr condition)))))))


(defvar paredit-strict-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map [remap delete-char] 'paredit-forward-delete)
    ...
    ;; (define-key map [remap backward-delete-char] 'paredit-backward-delete)
    ...
    map)
  "Keymap used for `paredit-strict-mode'.")

;;;###autoload
(define-minor-mode paredit-strict-mode
  "Toggle the strict paredit mode.
...
after the paredit indicator in the mode list."
  :init-value nil
  :group 'paredit
  (if paredit-strict-mode
	...
	(unless paredit-mode
          (paredit-mode 1))
	(unless (assq 'paredit-strict-mode minor-mode-overriding-map-alist)
          (push `(paredit-strict-mode . ,paredit-strict-mode-map)
                minor-mode-overriding-map-alist)))
    (setq minor-mode-overriding-map-alist
          (assq-delete-all 'paredit-strict-mode minor-mode-overriding-map-alist))))
...
(dolist (commands paredit-commands)
  (cl-loop for (key fn) in (cdr commands)
		   do (if (listp key)
				  (dolist (k key)
					(define-key paredit-mode-map (kbd k) fn))
				(define-key paredit-mode-map (kbd key) fn))))
  ...
  (let* ((regionp
    ...
    (let ((spacep (paredit-space-for-delimiter-p nil open)))
      ...
      (save-excursion
        ...
        (when (and (not (paredit-in-string-p))
                   (paredit-in-comment-p))
          (newline))
        (insert close)
...
If in a comment and if followed by invalid structure, call
  ...
  comment."
  (interactive)
  (cond ((paredit-in-string-p) (newline))
        ((paredit-in-comment-p)
...
If a list begins on the line after the point but ends on a different
  ...
  (interactive "p")
  (if (or (paredit-in-string-p)
	  (looking-at (rx (* space) (syntax comment-start))))
      (insert (s-repeat (or n 1) comment-start))
    (when (paredit-in-char-p)
      (backward-char 2))
    (or (if (not (paredit-in-comment-p))
	    (let ((line-break-point (paredit-semicolon-find-line-break-point)))
	      (when line-break-point
		(save-excursion
		  ;; let ((p (point)))
		  ...
		  (save-excursion
		    (newline)
		    (lisp-indent-line))
		  ;; This step is redundant if we are inside a list, but even if
		  ;; we are at the top level, we want at least to indent
		  ;; whatever we bumped off the line.
		  (paredit-ignore-sexp-errors (indent-sexp))
		  (paredit-indent-sexps))
		(comment-indent)
		;; done
		t)))
	(progn (insert (s-repeat (or n 1) comment-start))
	       (comment-indent)))))
           ...
           (catch 'exit
             (while t
               (let ((line-break-point (point)))
                 (cond ((paredit-ignore-sexp-errors (forward-sexp) t)
...
With a `C-u' prefix argument, simply delete a character forward,
  ...
  (cond ((or (consp argument) (eobp))
        ...
        ((paredit-in-string-p) (paredit-forward-delete-in-string))
        ((paredit-in-comment-p) (paredit-forward-delete-in-comment))
        ...
        ((integerp argument)
         (if (< argument 0)
           ...
           (while (> argument 0)
             ...
             (setq argument (- argument 1)))))
        ((paredit-in-string-p) (paredit-backward-delete-in-string))
        ((paredit-in-comment-p) (paredit-backward-delete-in-comment))
        ((paredit-in-char-p)            ; Escape -- delete both chars.
         (delete-char -1)
         (delete-char +1))
...
(defun paredit-forward-kill-word (&optional arg)
...
(defun paredit-check-region-for-delete:depth (start start-state end end-state)
  (let ((start-depth (nth 0 start-state))
    ...
    (or (= start-depth end-depth)
        (user-error "Mismatched parenthesis depth: %S at start, %S at end."
		    start-depth
		    end-depth))))
...
(defun paredit-check-region-for-delete:string (start start-state end end-state)
  (let ((start-string-p (nth 3 start-state))
    ...
    (or (eq start-string-p end-string-p)
        (user-error "Mismatched string state: start %sin string, end %sin string."
...
(defun paredit-check-region-for-delete:comment
  ...
  (let ((start-comment-state (nth 4 start-state))
    ...
    (or (eq start-comment-state end-comment-state)
        ...
        (user-error "Mismatched comment state: %s"
...
(defun paredit-check-region-for-delete:char-quote
  ...
  (let ((start-char-quote (nth 5 start-state))
    ...
    (when (not (eq start-char-quote end-char-quote))
      (let ((phrase "character quotation"))
        (user-error "Mismatched %s: start %sin %s, end %sin %s."
		    phrase
...
(defun paredit-forward (&optional arg)
...
(defun paredit-backward (&optional arg)
...
Inside a string, unescape all backslashes, or signal an error if doing
  ...
  (if (paredit-in-string-p)
    ...
    (if (paredit-in-comment-p)
        (user-error "Can't splice comment."))
    (paredit-handle-sexp-errors (paredit-enclosing-list-start)
      (user-error "Can't splice top level."))
...
(defun paredit-kill-surrounding-sexps-for-splice (argument)
  (cond ((or (paredit-in-string-p)
             (paredit-in-comment-p))
         (user-error "Invalid context for splicing S-expressions."))
  ...
  (save-excursion
    (cond ((paredit-in-string-p)
          ...
          ((paredit-in-char-p)
           (backward-sexp))
          ((paredit-in-comment-p)
           (user-error "No S-expression to raise in comment.")))
...
If in a string, move the opening double-quote forward by one
  ...
  (save-excursion
    (cond ((paredit-in-comment-p)
           (user-error "Invalid context for slurping S-expressions."))
          ((numberp argument)
           (if (< argument 0)
...
Both must be lists, strings, or atoms; error if there is a mismatch."
  (interactive)
  (paredit-lose-if-not-in-sexp 'paredit-join-sexps)
  ...
  (cond ((integerp argument) argument)
        ...
        ((consp argument)
         (cond ((equal argument '(4)) (paredit-count-sexps-forward))   ;C-u
               ...
               (t (user-error "Invalid prefix argument: %S" argument))))
        ((region-active-p)
         (save-excursion
           (save-restriction
...
(defun paredit-lose-if-not-in-sexp (command)
  (when (or (paredit-in-string-p)
            (paredit-in-comment-p)
            (paredit-in-char-p))
    (user-error "Not in sexp! Command `%s'." command)))
