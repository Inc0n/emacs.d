...
(defface history-current-history
  '((t (:foreground "black" :background "gold1" :weight bold)))
...
(defcustom history-window-local-history t
  "Window-local history has its preferences.
When set to non-nil it will use window-local history; otherwise
the global history will be used."
  :type 'boolean
  :group 'history)
...
(defvar history-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map [remap self-insert-command] 'ignored)
    (define-key map (kbd "<up>") 'ignored)
    (define-key map (kbd "<down>") 'ignored)
...
(defun history-add? (new-history)
  "Check readiness to add history, e.g. avoiding duplicates."
  (if history-stack
...
(defun history-window ()
  "Return variable `history-window' if minibuffer is active;
`selected-window' if inactive."
...
(defun history-sync-max ()
  ...
  (when (> (length history-stack) history-history-max)
 ...
 (0) (1) (new)
  ...
  (when (history-add? history)
    (if history-from-scratch?
      ...
      (when (and history-stack (>= history-index 1))
		...
		(cl-remove-if (lambda (history)
						(let* ((marker (plist-get history :marker))
						  ...
						  (or
						   ...
						   (let* ((symbol (plist-get history :symbol))
								  (pos (marker-position marker)))
							 (with-current-buffer buffer
							   (save-excursion
								 (goto-char pos)
								 (not
								  (equal symbol (history-thingatpt 'symbol))))))
						   ;; Remove temporary history.
						   (and remove-temp? (plist-get history :temp)))))
					  history-stack))
...
(defun history-histories-string ()
  ...
  (let ((total (length history-stack)))
    (concat
	 (propertize						; prompt
	  (format "History %d/%d: "
              (1+ (or history-index 0)) total)
      'face 'history-prompt)
	 ;; history progress bar
	 ;; (let ((bar (make-string total ?.))))
	 (concat (make-string history-index ?.)
			 (if (plist-get (nth history-index history-stack) :temp)
				 (propertize "*" 'face 'history-current-temp-history)
			   (propertize "*" 'face 'history-current-history))
			 (make-string (- total history-index 1) ?.))
	 ;; (mapconcat (lambda (history)
	 ;; 			  (if (eq history (nth history-index history-stack))
	 ;; 				  (if (plist-get history :temp)
	 ;; 					  (propertize "*" 'face 'history-current-temp-history)
	 ;; 					(propertize "*" 'face 'history-current-history))
	 ;; 				(or "."
	 ;; 					(if (plist-get history :temp)
	 ;; 						(propertize "." 'face 'history-temp-history)
	 ;; 					  (propertize "." 'face 'history-other-history)))))
	 ;; 			history-stack
	 ;; 			"")
	 )))
...
(defun history-preview-prev-history ()
  ...
  (setq history-index (max 0 (1- history-index)))
...
(defun history-preview-next-history ()
  ...
  (interactive)
  (delete-minibuffer-contents)
  (setq history-index (min (1- (length history-stack))
						   (1+ history-index)))
  (insert (history-histories-string))
  ;; (re-search-forward "\*")
  ;; Use history.
...
(defun history--pre-cmd-hook ()
  (when (memq this-command history-advised-before-functions)
	(history-add-history t)))

(defun history--post-cmd-hook ()
  (when (memq this-command history-advised-after-functions)
	(history-add-history t)))
...
See `history-advised-before-functions'
  ...
  (if activate?
	  (progn
		(add-hook 'pre-command-hook 'history--pre-cmd-hook)
		(add-hook 'post-command-hook 'history--post-cmd-hook))
	(remove-hook 'pre-command-hook 'history--pre-cmd-hook)
	(remove-hook 'post-command-hook 'history--post-cmd-hook)))
  ...
  (history-do
    (when history-stack
      (let* ((cached-history-index history-index)
        ...
        (history-use-current-history)
		(condition-case nil
			(read-from-minibuffer "" (cons str index) history-map)
		  (error
		   (progn
    ...
    (when history-stack
      ...
      (let* ((history (nth history-index history-stack))
		...
		(unless (eq buffer (current-buffer))
		  (user-error "History is a different buffer"))
		(unless (history-same-line? (point) pos)
		  (message "History was not in the same line"))
        (history-move-history -1))
      ;; Use history.
      (history-use-current-history))
    (message (history-histories-string))))
