...
(declare-function inf-haskell-get-result "ext:inf-haskell" (expr &optional process))
...
(defun org-babel-execute:haskell (body params)
  "Execute a block of Haskell code BODY with PARAMS."
    ...
    (org-babel-reassemble-table
	 (let ((result
			(cl-case result-type
			  (output results)
			  (value results))))
	   (org-babel-haskell-table-or-string result)
...
then create one.  Return the initialized session."
  ...
  (or (get-buffer "*haskell*")
      (save-window-excursion
		(run-haskell) (sleep-for 0.25) (current-buffer))))
  ...
  (save-window-excursion
    (let* ((buffer (org-babel-prep-session:haskell session params))
      ...
      (with-temp-buffer
        ...
        (haskell-mode 1) (inf-haskell-load-file loaf-file))
      buffer)))
