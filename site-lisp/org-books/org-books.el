...
;; This is my clone that doesn't use helm or helm related packages
...
cursor to add log entry."
  (let ((active-books (org-books--get-active-books)))
    (if (null active-books)
      ...
      (let ((picked-book-pos
             (let* ((header (completing-read "Active books" active-books))
                    (mark (cdr (assoc header active-books))))
               mark)))
        (find-file org-books-file)
        (goto-char picked-book-pos)
      ...
      (save-excursion
        (with-current-buffer (find-file-noselect org-books-file)
          (let ((headers (org-books-get-headers)))
            (if headers
                (let* ((header (completing-read "org-book categories" headers))
                       (mark (cdr (assoc header headers))))
                  (org-books--insert-at-pos (marker-position mark) title author props))
              (goto-char (point-max))
