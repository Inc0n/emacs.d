...
(defcustom vr/match-string-face '(:inherit error :strike-through t)
  ""
  :group 'visual-regexp)

(defcustom vr/replace-string-face '(:inherit compilation-info)
  ""
  :group 'visual-regexp)
;; custom-themed

(defcustom vr/match-separator-custom-face nil
...
(defvar vr--regexp-string nil "Entered regexp.")

(defvar vr--replace-string nil "Entered replacement.")
    ...
    (let* ((overlay (vr--get-overlay i 0))
      ...
      (let ((current-face (nth (mod i (length vr--match-faces)) vr--match-faces)))
        (if (or empty-match vr--replace-preview)
            (progn
              (overlay-put overlay
						   (if empty-match 'after-string 'display)
						   (propertize replacement 'face current-face))
              (overlay-put overlay 'priority (+ vr--overlay-priority 2)))
		  (overlay-put overlay 'face vr/match-string-face)
          (overlay-put overlay 'after-string
                       (concat (if vr/match-separator-string
								   (propertize vr/match-separator-string 'face
											   vr/match-separator-custom-face))
                               (propertize replacement
										   'face vr/replace-string-face)))
          (overlay-put overlay 'priority vr--overlay-priority))))))
          ...
          (with-current-buffer vr--target-buffer
            ...
            (let ((looping t))
              (while (and
                      looping
                      (condition-case err
                          (funcall (if vr/plain 'search-forward 're-search-forward))
                        (invalid-regexp 
						 (setq message-line (cadr err))
						 nil)))
                (condition-case err
                    (progn
                      (if (or (not feedback) (not feedback-limit) (< i feedback-limit))
                          (setq replacements
								(cons
                                 (let ((match-data
										(vr--mapcar-nonnil
										 'marker-position (match-data))))
                                   (list (query-replace-compile-replacement
										  replace-string
										  t)
										 match-data i))
                                 replacements))
                  ...
                  (error 
                   ...
                   (setq looping nil)))))))
      (invalid-regexp (setq message-line (cadr err)))
    ...
    (if feedback
        (if (string= "" message-line)
            (setq message-line
				  (vr--compose-messages
				   (format "%s matches" i)
				   (when vr--limit-reached (format "%s matches shown, hit C-c a to show all" feedback-limit)))))
...
(defun vr--do-replace (&optional silent)
  ...
  (vr--delete-overlays)
  (replace-string vr--regexp-string vr--replace-string)
  ...
  ;;     (set-match-data
  ;; 	   (vr--mapcar-nonnil
  ;; 		(lambda (el) (+ cumulative-offset el)) last-match-data))
  ;;     replace-count))
  )
