...
;;; TODO:
;; - KNOWN Fix from string faces, when separator has been deleted
;;   cause -> visual-replace--overlay
;; - TODO move cursor to next match, while visual querying, similar to next TODO
;;   NEED to store old cursor position to restore, like isearch
;; - TODO implement query-replace interface on finish
;;   NEED to find a way to get all instances, or just replace current and repaint
...
(defcustom visual-replace-keep-incomplete nil
...
(defcustom visual-replace-min-from-length 0
  "Minimum length until preview shows.
...
(defvar visual-replace-secondary-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map "r" 'visual-replace-toggle-regexp)
    (define-key map " " 'visual-replace-toggle-scope)
    (define-key map "q" 'visual-replace-toggle-query)
    (define-key map "w" 'visual-replace-toggle-word)
    (define-key map "c" 'visual-replace-toggle-case-fold)
    (define-key map "s" 'visual-replace-toggle-lax-ws)
    map)
...
(defvar visual-replace-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map [?\C-t] visual-replace-secondary-mode-map)
    map)
  ...
  (let ((args (visual-replace-args--from-minibuffer)))
    (if (visual-replace-args-regexp args)
      ...
      (setf (visual-replace-args-regexp args) t
	    (visual-replace-args-word args) nil))
  ...
  (let ((args (visual-replace-args--from-minibuffer)))
    (if (visual-replace-args-word args)
      ...
      (setf (visual-replace-args-word args) t
	    (visual-replace-args-regexp args) nil))
    (visual-replace--update-separator args 'forced)))
...
(defun visual-replace-toggle-lax-ws ()
  ...
  (let* ((args (visual-replace-args--from-minibuffer))
    ...
    (setf (visual-replace-args-lax-ws-regexp args) newval
	  (visual-replace-args-lax-ws-non-regexp args) newval)
    (visual-replace--update-separator args 'forced)))
...
specified.  It must be a `visual-replace-args' struct."
  ...
  (let ((history-add-new-input nil)
    ...
    (save-excursion
      (unwind-protect
          (minibuffer-with-setup-hook
              (lambda ()
		(when (or visual-replace-preview
			  visual-replace-keep-incomplete)
		...
		(when (and initial-input
			   (not (string-empty-p (car initial-input))))
		  (visual-replace--after-change)))
	...
	(remove-hook 'after-change-functions 'visual-replace--after-change
		     'local)))
...
;;;###autoload
(defun visual-replace-regexp (args ranges)
  (interactive (visual-replace-read
		(visual-replace-make-args
		 :regexp t
		 :word (and current-prefix-arg
...
Adds the overlays to `visual-replace--overlays'"
  (let* ((preview-start (get-internal-run-time))
	 ...
	 (total-replaced 0))
    ;; (message "visual-replace: %S"
    ;; 		 (list (visual-replace-args-regexp args)
    ;; 			   (visual-replace-args-word args)))
...
call is a set of overlays, stored in `visual-replace--overlays'."
  ...
  (let* ((args (visual-replace-args--from-minibuffer))
    ...
    ;; (when (> (length (visual-replace-args-from args))
    ;; 		 visual-replace-min-from-length))
    (with-current-buffer visual-replace--calling-buffer
      (condition-case nil
...
(defun visual-replace--clear-preview ()
  ...
  (with-current-buffer visual-replace--calling-buffer
    (mapc #'delete-overlay visual-replace--overlays))
...
(defun visual-replace--overlay-at-p (pos)
  ...
  (cl-dolist (ov (overlays-at pos))
    (if (eq (overlay-get ov 'visual-replace)
	    t)
	(cl-return t))))
